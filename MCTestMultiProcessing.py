...
def test1(randnumbers):
    ...
    print('Time taken multiprocessing ',stop-start) 


if __name__ == '__main__':
    #Creating points with random numbers
    LCGgeneratorpoints=LCG(1,1103515245,12345,2**32)
    RandomNumberList=LCGgeneratorpoints.numberSequence(2000000)

    SCGgeneratorpoints=SCG(2,2**32)  
    RandomNumberList1=SCGgeneratorpoints.getSequence(2000000)
    
    jobs = []
    for i in [RandomNumberList,RandomNumberList1]:
